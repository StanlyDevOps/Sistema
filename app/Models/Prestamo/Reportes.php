        ...
        try {
            return DB::select(
                "SELECT pc.identificacion,
                        ...
                        pp.no_cuotas + 1 -
                        (
                            SELECT ppd1.no_cuota
                            FROM p_prestamo_detalle ppd1
                            WHERE ppd1.id_prestamo = pp.id
                            AND ppd1.valor_pagado < ppd1.saldo_final
                            LIMIT 1
                        )                                     AS cuotas_faltantes,
                ...
                INNER JOIN p_cliente        pc  ON  pp.id_cliente     = pc.id
                                                AND pp.estado         = pc.estado
                INNER JOIN s_transacciones st   ON pp.id            = st.id_alterado
                                                AND st.id_permiso     = 1
                                                AND st.id_modulo      = 32
