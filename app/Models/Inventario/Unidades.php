...
class Unidades extends Model
{
    ...
    const MODELO = 'Unidades';

    # Belongs to
    public function productos() {
        return $this->belongsTo(Producto::class, 'id_unidades');
    }
    ...
    public static function ConsultarTodo($request, $buscar = null, $pagina = 1, $tamanhio = 10, $idEmpresa) {
        try {
            ...
            return Unidades::whereRaw("unidad like '%$buscar%'")
                ->where('estado','>','-1')
     ...
     * @return object
     */
    public static function ConsultarPorUnidad($request, $idEmpresa, $unidad) {
        try {
            return Unidades::where('id_empresa',$idEmpresa)
                ->where('unidad',$unidad)
                ->get();
        } catch (\Exception $e) {
            $hs = new HerramientaStidsController();
     ...
     * @autor: Jeremy Reyes B.
     * @version: 1.0
     * @date_create: 2018/05/07 - 8:31 PM
    ...
    public static function FindActive($data, $idCompany) {
        try {
            return Unidades::where('id_empresa',$idCompany)
                ->where('estado', '1')
                ->orderBy('unidad')
                ->get();
