...
class HerramientaStidsController extends Controller
{
    ...
    public $imagesPR = null;


    public function __construct()
    {
        $this->imagesPR = base_path() . '/../recursos/imagenes';
    }
    ...
    public static function Save($clase,$mensaje,$transaccion = '') {
        return self::ejecutarSave($clase,$mensaje,$transaccion);
    }
     ...
     * @param object    $request:        Peticiones realizadas.
     *
     * @return object
     */
     ...
     * @autor: Jeremy Reyes B.
     * @version: 1.0
     * @date: 2018-05-01 - 09:03 AM
     *
     * Upload a file to a specific directory
     *
     * @param string $directory:        Path
     * @param string $previusFile:      Previus file
     * @param object $file (null):      Object file
     * @param string $company (null):   ID company.
     * @param integer $id (null):       ID.
     *
     * @return bool || string
     */
    public function UploadFile($directory, $previusFile, $file = null, $company = null, $id = null) {

        if (is_null($file)) {
            $id       = request()->all()['id'];
            $file     = request()->file()['file'];
            $company  = request()->session()->get('idEmpresa');
        }

        $name     = explode('.',$file->getClientOriginalName());
        $fileName = $company . "_" . $id . "_" . date("Ymd_his") . ".{$name[count($name) - 1]}";

        if ($file->move($directory, $fileName)) {

            if ($previusFile !== 'default.png') {
                @unlink("{$directory}/{$previusFile}");
            }
     ...
     * @autor: Jeremy Reyes B.
     * @version: 1.0
     * @date_create: 2018/05/14 - 4:21 PM
