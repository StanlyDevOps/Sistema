...
class UnidadesController extends Controller
{
    ...
    public function __construct()
    {
        ...
        self::$transaccion = ['', 66, '', 'inv_unidades'];
    }
    ...
    public function CrearActualizar(Request $request)
    {
        ...
        $datos = [
            'unidad' => 'Debe digitar la unidad para poder guardar los cambios'
        ];
        ...
        if (!$id) {
            $existeRegistro = Unidades::ConsultarPorUnidad(
                $request,
                $idEmpresa,
                $request->get('unidad')
            );
            ...
            $existeRegistro[] = Unidades::find($id);
        }
        ...
        if (!is_null($existeRegistro)) {
            ...
            elseif ($id || $existeRegistro->count() && $existeRegistro[0]->estado < 0) {
                ...
                $clase = $this->insertarCampos(Unidades::find($existeRegistro[0]->id), $request);
            ...
            #3.3. Si no existe entonces se crea
                ...
                $clase = $this->insertarCampos(new Unidades(), $request);
    ...
    private function insertarCampos($clase,$request) {
        ...
        $clase->unidad      = $request->get('unidad');
    ...
    public function CambiarEstado(Request $request) {
        ...
        $clase  = Unidades::Find((int)$request->get('id'));
