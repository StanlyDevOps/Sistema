Api.PrestamoDetalle = {
    ...
    constructor: function() {
        ...
        this._GuardarPagoCuota      = this.$uriCrud('GuardarPagoCuota',this.controlador,this.carpeta);
    ...
    cargarInformacion: function() {
        ...
        $('#detalle-total-mora').text($AHfM(this.$informacion.total_mora));
    ...
    ampliar: function(id,$informacion) {
        ...
        $('#ampliar-saldo').val(Api.Herramientas.formatoNumerico($informacion.intereses));
        $('#ampliar-total').val('');
    ...
    pagarCuota: function(id) {
        ...
        $('#modal-pagos')
            ...
            .find('#btn-guardar')
            .attr('onclick','Api.PrestamoDetalle.guardarPagoCuota()');
    },

    guardarPagoCuota: function() {

        var $parametros = this.verificarFormularioPagos(this._GuardarPagoCuota);
        var $objeto     = Api.PrestamoDetalle;
        var $AP         = Api.Prestamo;

        $parametros['id_prestamo_detalle'] = this.id;
    ...
    detalle: function(id, $informacion) {
        ...
        var contenedor = '#modal-detalle-prestamo ',
            ...
            $APDP = Api.PrestamoDetallePago;

        $(contenedor + '.cuota').text($informacion.no_cuota);
        $(contenedor + '.fecha').text($informacion.fecha_pago);
        $(contenedor + '.saldo-inicial').text($AHfM($informacion.saldo_inicial));
        $(contenedor + '.cuota-pagar').text($AHfM($informacion.cuota_a_pagar));
        $(contenedor + '.interes').text($AHfM($informacion.intereses));
        $(contenedor + '.abono-capital').text($AHfM($informacion.abono_capital));
        $(contenedor + '.valor-pagado').text($AHfM($informacion.valor_pagado));
        $(contenedor + '.estado').text($informacion.estado_pago);

        $APDP.idPrestamoDetalle = $informacion.id;
        $APDP.constructor();

        $('#modal-detalle-prestamo').modal({backdrop: 'static', keyboard: false});
    },
        ...
        return {
            parametrizacion: [
                ...
                },
                {
                    nombre: 'Pagar cuota',
                    icono: 'fa-money',
                    accion: 'Api.' + this.controlador + '.pagarCuota',
                    color: '#1ab394',
                    estado: false,
                    permiso: 'actualizar',
                    informacion: false
